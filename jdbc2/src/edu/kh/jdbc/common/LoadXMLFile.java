...
public class LoadXMLFile {
	public static void main(String[] args) {
		...
		try {
			...
			// Properties 객체에 K:V 형식으로 저장하겠다
			prop.loadFromXML(fis);
			
			System.out.println(prop);
			
			// Property : 속성(데이터)
			// prop.getProperty("key") : key가 일치하는 속성(데이터)을 얻어온다
			...
			System.out.println(conn);  // Connection 생성 확인
			
			/* 왜 XML파일을 이용해서 DB 연결 정보를 읽어와야될까?
			   
			   1. 코드 중복 제거
			   2. 별도 관리 용도 - 별도 파일을 이용해서 수정이 용이함
			   3. 재 컴파일을 진행하지 않기 위해서
			     - 코드가 길수록 컴파일에 소요되는 시간이 크다
			     -> 코드 수정으로 인한 컴파일 소요시간을 없앰
			     	(파일의 내용을 읽어오는 코드만 작성해두면 JAVA 코드 수정 없이,
			     	 파일 내용만 수정하면 재 컴파일은 수행되지 않는다)
