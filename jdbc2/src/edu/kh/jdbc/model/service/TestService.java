...
// 하나의 Service 메서드에서 n개의 DAO 메서드(지정된 SQL을 수행)를 호출하여
// 이를 하나의 트랜잭션 단위로 취급하여 한 번에 commit, rollback을 수행할 수 있다
...
public class TestService {
	 ...
	 * @param vo1
	 * @return result
	 * @throws SQLException  // DAO에서 받아서 RUN으로 또 던짐
	 */
	public int insert(TestVO vo1) throws SQLException {
		
		// 첫줄 : 커넥션 생성
		// static은 클래스명.메서드명으로 호출 가능
		...
		if(result > 0) commit(conn); //JDBCTemplate.commit(conn)
		else		   rollback(conn);
		
		// return 전 : 커넥션 반환(close)
	...
	public int insert(TestVO vo1, TestVO vo2, TestVO vo3) throws Exception {
		...
		int res = 0; // insert 3회 모두 성공 시 1, 아니면 0 저장
		...
		try {
			...
			if(result1 + result2 + result3 == 3) { // 모두 insert 성공한 경우
				...
				rollback(conn); 
				// res 초기값이 0이기 때문에 별도 표기 필요X
			}
			
		} catch (SQLException e) { // dao수행 중 예외 발생 시
			rollback(conn);
			
			// -> 실패된 데이터는 DB에 삽입하지 않음
