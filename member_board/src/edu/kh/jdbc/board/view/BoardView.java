...
public class BoardView {
	...
	private void selectAllBoard() {
		...
		try {
			...
			// -> DAO에서 new ArrayList<>(); 구문으로 인행 반환되는 조회 결과는
			//    null이 될 수 없다.
			
			if(boardList.isEmpty()) { // 조회 결과가 없는 경우
				System.out.println("게시 글이 존재하지 않습니다.");
			} else {
	...
	private void subBoardMenu(Board board) {
		...
		try {
			...
			// 로그인한 회원과 게시글 작성자가 같을 때만 출력되는 메뉴
			...
			// 로그인한 회원의 회원 번호 미리 저장
			// static에 있어서 바로 써도 되지만 길어서 memberNo에 저장해줌
			...
			case 4 : case 5 :  // 4 또는 5 입력 시
				...
				if(MainView.loginMember.getMemberNo() == board.getMemberNo()) {
					...
					break; // switch문 종료
					// if문이 실행 안되면 바로 default로 넘어감
					// if문이 실행되면 default로 넘어가지 않기 위해 break 써준다
				}
			...
			// 메서드가 자기 자신 계속 호출하는 것
			if(input > 0 && input < 5) { // 1,2,3,4 번 선택할 때만 재귀호출,
										// 5번 선택하면 삭제해서 null이 발생할 수 있기 때문에
										// 재귀호출이 아닌 바로 게시판 메뉴가 나오게 하는 것
				try {
	...
	/** 댓글 등록
	 * @param bNo = boardNo
	 * @param mNo = memberNo (case1에서 넘어왔지만 자료형만 같으면 이름은 달라도 된다)
	 */
	...
	private String inputContent() {
		...
		// 문자열을 참조하지만 그 안에 내용이 없다(참조는 하고 있는 것)
	...
	private void updateBoard(int boardNo) {
		try {
			...
			// 수정된 내용/제목 + 게시글 번호를 한 번에 전달하기 위한 Board 객체 생성
	...
	private void searchBoard() {
		try {
			...
			if(condition >=1 &&  condition <= 4) {//정상입력
