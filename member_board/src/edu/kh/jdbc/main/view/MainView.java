...
//	private Member loginMember = null;
	public static Member loginMember = null;  // 다른 곳에서 접근 가능하게 만듦
		...
		do {
			try {
				...
				if (loginMember == null) {
					...
					// 회원 기능 서브 메뉴 출력(매개변수도 같이 전달)
					case 1: memberView.memberMenu(loginMember); break;
					
					case 2: boardView.boardMenu(); break;
					// 매개 변수로 loginMember 넘기지 않고,
					// 회원 정보가 필요한 경우에 static에서 얻어와 사용할 예정
					...
					case 99 :
							...
							System.exit(0);  // JVM 종료, 매개변수는 0, 아니면 오류를 의미(비정상적 종료)
							break;
					default: System.out.println("메뉴에 작성된 번호만 입력해주세요.");
					} 
				}
	...
	private void signUp() {
		...
		try {
			...
			// -> 하나의 VO에 담아서 서비스를 호출 후 결과 반환 받기
