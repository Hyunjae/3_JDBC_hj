...
-- 사용자 계정 생성
CREATE USER member_jhj IDENTIFIED BY member1234;

-- 생성한 사용자 계정에 권한 부여
GRANT CONNECT, RESOURCE, CREATE VIEW TO member_jhj;

-- 테이블 스페이스 할당
ALTER USER member_jhj DEFAULT TABLESPACE SYSTEM QUOTA UNLIMITED ON SYSTEM;
...
-- 가입일 내림차순이지만 날짜 비교보다는 숫자 비교가 빠르다
...
MEMBER_NM = '바꾼이름',  -- 입력
MEMBER_GENDER = 'F'		 -- 입력
WHERE MEMBER_NO = 6;	 -- loginMemeber.getMemberNo();
...
-- 비밀번호 수정
UPDATE MEMBER SET MEMBER_PW = 'pass04'
WHERE MEMBER_NO = 6
AND MEMBER_PW = 'pass1234';
...
INSERT INTO "COMMENT" 
VALUES (SEQ_COMMENT_NO.NEXTVAL, '댓글 샘플 4번', DEFAULT, DEFAULT, 3, 3);

DELETE FROM "COMMENT" WHERE COMMENT_NO = 4;
...
--
	CASE	
		WHEN SYSDATE - CREATE_DT < 1/24/60  --1분 미만, 몇 초 전
		THEN FLOOR( (SYSDATE - CREATE_DT) * 24 * 60 * 60) || '초 전'   
		WHEN SYSDATE - CREATE_DT < 1/24  --60분 미만, 몇 분 전
		THEN FLOOR( (SYSDATE - CREATE_DT) * 24 * 60 ) || '분 전'
		WHEN SYSDATE - CREATE_DT < 1  --60분 미만, 몇 시간 전
...
UPDATE "COMMENT" SET COMMENT_CONTENT = '댓글수정테스트'
WHERE COMMENT_NO = 3; 
...
WHERE DELETE_FL = 'N'
-- 제목
--AND BOARD_TITLE LIKE '%' || ? || '%' --STRING을 넣으면 자동으로 ''붙여줘서 연결연산자 사용
-- 내용
--AND BOARD_CONTENT LIKE '%' || ? || '%'
-- 제목+내용 (둘 중 하나에 포함)
--AND (BOARD_TITLE LIKE '%' || '1234' || '%'
--OR BOARD_CONTENT LIKE '%' || '1234' || '%')
-- 작성자
AND MEMBER_NM LIKE '%' || '육' || '%'
ORDER BY BOARD_NO DESC;
